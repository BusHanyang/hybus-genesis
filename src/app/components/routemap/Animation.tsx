...
const circleAnimation = (props: CircleAnimate) => {
  ...
  for (
    let i = props.index;
    i <= props.index + 1 && props.ref.current[i].childNodes.length <= 1;
    i++
  ) {
    ...
    pingCircle.classList.add(
      ...
      'rt1:w-2.5',
      'z-[1]',
      i === 5 || (i === 4 && props.chipColor === 'bg-chip-blue')
        ? 'bg-chip-orange'
        : props.chipColor,
      'mx-2',
    )
...
const timetableType = (type: string, index: number, dotall: DotProps) => {
  if (type === 'C')
    return { ref: dotall.dotCyc, index: index, chipColor: 'bg-chip-red' }
  else if (type === 'DHJ')
    return { ref: dotall.dotJun, index: index, chipColor: 'bg-chip-purple' }
  else if (type === 'DY')
    return { ref: dotall.dotYes, index: index, chipColor: 'bg-chip-green' }
  else return { ref: dotall.dotDir, index: index, chipColor: 'bg-chip-blue' }
}

const circleAnimationRemove = (
  refs: React.MutableRefObject<NodeListOf<HTMLDivElement> | undefined>,
) => {
    ...
    switch (tab) {
      ...
      case 'shuttlecoke_i':
        ...
        circleAnimation({
          ref: dots.dotCyc,
          index: 4,
          chipColor: 'bg-chip-red',
        })
        circleAnimation({
          ref: dots.dotYes,
          index: 4,
          chipColor: 'bg-chip-green',
        })
        break
      default:
